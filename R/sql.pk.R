...
sql.attribute.characteristics <- function(query, query.colname, query.countname, query.nullitymarker){
  single.replacer <- function(query, query.colname, query.countname, query.nullitymarker){
    stringi::stri_replace_all_fixed(
      rep(get("attribute_primarykeyability", pkg_globals), length(query)), 
      c(
        "%%QUERY%%", "%%VALNAME%%", "%%VALCOUNTER%%", "%%NULLITYMARKER%%"
      ), 
      c(
        query, query.colname, query.countname, query.nullitymarker
      ),
      vectorize_all = FALSE)       
  }
  mapply(single.replacer, query, query.colname, query.countname, query.nullitymarker)
}
